...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Sử dụng MySQLi Prepared Statements để tránh SQL Injection
    $stmt = $connect->prepare("SELECT * FROM taikhoannhanvien WHERE TenDangNhap = ? AND MatKhau = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION["TaiKhoanID"] = $row["TaiKhoanID"];
        ...
        elseif($row["role"] == 2) {
            header("Location: index_ketoan.php");
            exit;
        }
    } else {
    ...
    <title>Đăng nhập</title>
        ...
        *{
        ...
        body{
            ...
            background: liner-gradient(to right, #e2e2e2, #c9d6ff);
            display: flex;
            align-items: center;
        ...
        .toggle{
            background-color: #5fcf86;
            height: 100%;
            background: linear-gradient(to right, #0c8a2b, #5fcf86);
            color: #fff;
            position:  relative;
            left: -100%;
            height: 100%;
        ...
        <div class="toggle-container">
            <div class="toggle">
                ...
                <div class="toggle-panel toggle-right">
                   ...
                   <a href="dangky.php"><button class="register">Đăng Ký</button></a> 
                </div>
            </div>
        </div>
    </div>
